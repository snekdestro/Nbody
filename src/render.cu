...
void g_framebuffer_size_callback(GLFWwindow* window, int width, int height) {
...
{   
	...
	GLFWwindow* window = glfwCreateWindow(g_WINDOW_WIDTH,g_WINDOW_HEIGHT, "OpenGL 4.6 Template", NULL, NULL);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        float* d_ptr;
        size_t size;
		m_x = (2.0f * xpos ) / g_WINDOW_WIDTH - 1.0f;
        m_y = 1.0f - (2.0f * ypos) / g_WINDOW_HEIGHT;
...
int e_WINDOW_HEIGHT = 1080;
int e_WINDOW_WIDTH = 1080;



void e_framebuffer_size_callback(GLFWwindow* window, int width, int height) {
	glViewport(0, 0, width, height);
	e_WINDOW_WIDTH = width;
	e_WINDOW_HEIGHT = height;
}
...
{   
	...
	GLFWwindow* window = glfwCreateWindow(e_WINDOW_WIDTH,e_WINDOW_HEIGHT, "OpenGL 4.6 Template", NULL, NULL);
	glfwMakeContextCurrent(window);
    gladLoadGLLoader((GLADloadproc)glfwGetProcAddress);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        float* d_ptr;
        size_t size;
		m_x = (2.0f * xpos ) / e_WINDOW_WIDTH - 1.0f;
        m_y = 1.0f - (2.0f * ypos) / e_WINDOW_HEIGHT;
        ...
        cudaDeviceSynchronize();
        cudaGraphicsUnmapResources(1, &cuda_vbo_resource, 0);
        cudaMemcpy(c_e_field, ee, 2 * sizeof(float), cudaMemcpyDeviceToHost);

  
        line_vertices[2] = m_x + c_e_field[0];
        line_vertices[3] = m_y + c_e_field[1];
        float currentFrame = glfwGetTime();
        dt = currentFrame - lastFrame;
        lastFrame = currentFrame;
        glClear(GL_COLOR_BUFFER_BIT); // Clear the previous frame
